...
| Let (s, If (_, _, rts1, rts2)) ->
  12 + count_lets_in_regt_list rts1 + count_lets_in_regt_list rts2
| Exp (If (_, _, rts1, rts2)) ->
  11 + count_lets_in_regt_list rts1 + count_lets_in_regt_list rts2
| Let (s, _) -> 1
| _ -> 0
    ...
    (match expr with
      ...
      | Call (func_name) ->
        ...
        let instructions =
          ...
          Printf.sprintf "\tmov %s, r0" s;] @ instructions
  ...
  (match exp with
    ...
    | Call (func_name) ->
      ...
      let instructions =
        ...
        [Printf.sprintf "\tpush {r0-r10}";
        Printf.sprintf "\tbl %s" func_name;
        Printf.sprintf "\tpop {r0-r10}"] @ instructions
